...
const CHAT_HISTORY_KEY_PREFIX = 'seller_chat_history_';
const DAYS_TO_KEEP = 30;

class SellerChatHistoryService {
  ...
  async saveMessage(message: Omit<ChatMessage, 'timestamp'>): Promise<void> {
    try {
      ...
      console.error('Error saving seller message:', error);
      throw error;
    }
  }
  ...
  async loadRecentMessages(): Promise<ChatMessage[]> {
    try {
      ...
      console.error('Error loading seller recent messages:', error);
      return [];
    }
  }
  ...
  async clearHistory(): Promise<void> {
    try {
      ...
      console.error('Error clearing seller chat history:', error);
      throw error;
    }
  }
}

export const sellerChatHistoryService = new SellerChatHistoryService();
